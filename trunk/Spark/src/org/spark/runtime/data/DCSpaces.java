	...
	public DataObject collect0(SparkModel model) throws Exception {
		...
		for (String name : names) {
			...
			if (space instanceof BoundedSpace) {
				...
				min = new Vector(boundedSpace.getXMin(), boundedSpace.getYMin(), 0);
				max = new Vector(boundedSpace.getXMax(), boundedSpace.getYMax(), 0);
			}
			else if (space instanceof BoundedSpace3d) {
				BoundedSpace3d boundedSpace3d = (BoundedSpace3d) space;
				min = new Vector(boundedSpace3d.getXMin(), boundedSpace3d.getYMin(), boundedSpace3d.getZMin());
				max = new Vector(boundedSpace3d.getXMax(), boundedSpace3d.getYMax(), boundedSpace3d.getZMax());
			}
